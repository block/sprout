...
The encoded base64 line MUST NOT exceed 2048 bytes (sufficient for the 1024-byte
...
- Line-wrapped base64.
- Base64 line exceeding 2048 bytes.
...
|-------|---------|-------------|-------------|
| `v`   | integer | MUST be `1` | Schema version. |
| `pk`  | string  | Exactly 64 lowercase hex characters. MUST be a valid BIP-340 x-only public key (i.e., the x-coordinate of a point on the secp256k1 curve). | Signer's public key. |
| `sig` | string  | Exactly 128 lowercase hex characters. | BIP-340 Schnorr signature. |
| `t`   | integer | MUST be in the range 0 to 4294967295. MUST NOT be negative, a float, or a string. | Claimed unix timestamp (seconds) of the signing event. See Security Considerations for implications of signer-controlled timestamps. |

JSON parsing rules:
- The content MUST be a single JSON object (not an array, string, or primitive).
- Duplicate keys: if present, verifiers MUST reject the signature.
- For `v=1`, unknown keys MUST cause rejection. Future versions (`v=2`, etc.)
  define their own field sets. This prevents unsigned extension fields from
  being injected into the envelope.
- The total decoded JSON MUST NOT exceed 1024 bytes.
- Implementations MUST reject non-canonical hex (uppercase, odd-length, whitespace).

### Signing Hash

The timestamp `t` is included in the hash preimage so that it is
cryptographically bound to the signature. Tampering with `t` invalidates the
signature.

Given a git object payload (the bytes git pipes to stdin) and a signing
timestamp `t`:

```
hash = SHA-256( "nostr:git:v1:" || decimal(t) || ":" || payload_bytes )
```
...
- `":"` is a single colon byte (`3a`), separating the timestamp from the payload.
...
3. Compute `hash = SHA-256("nostr:git:v1:" || decimal(t) || ":" || payload)`.
...
5. Construct the JSON object with compact serialization (no whitespace):
   `{"v":1,"pk":"<hex>","sig":"<hex>","t":<integer>}`.
   Field order MUST be `v`, `pk`, `sig`, `t`.
...
2. Base64-decode the middle line. Parse as JSON.
...
7. Compute `hash = SHA-256("nostr:git:v1:" || decimal(t) || ":" || payload)`.
...
```
Where `<key_id>` is the `pk` field if parseable, or `0000000000000000` if not.
The trailing fields are fixed placeholders (algo, hash algo, class, timestamp,
rc=9 meaning "no public key" / general error).
...
3964363966376362343631303031 0a 617574686f7220546573742055736572
...
- JSON with unknown keys (for `v=1`) — `ERRSIG`, exit 1.
...
- Decoded JSON exceeding 1024 bytes — `ERRSIG`, exit 1.
...
The 1024-byte JSON limit, 2048-byte base64 limit, and 100 MB payload limit
...
| NIP-OA | Owner attestation. NIP-OA proves an agent acts on behalf of an owner; this NIP proves the agent authored a specific commit. |
