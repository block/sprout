...
The phrase **configured relays** used throughout this NIP means the agent's write relays as advertised in its [NIP-65](65.md) `kind:10002` relay list (the `pubkey_a` is the author of every record), or an out-of-band configuration when no such list is published. Owners locate an agent's memory by reading the same relay list.
...
4. **Verify.** Allow time for the published event to propagate (implementations choose a timeout; conflicts undetected within that window are treated as eventually-consistent and surfaced by the next read), then recompute the head of `s`. If it is not the event just published, treat as a **conflict** and surface to the caller; clients MUST NOT silently retry.
