...
SIDECARS=(sprout-acp sprout-mcp-server git-credential-nostr git-sign-nostr)
...
if [[ ${#missing[@]} -gt 0 ]]; then
    ...
    echo "Run 'cargo build --release -p sprout-acp -p sprout-mcp -p git-credential-nostr -p git-sign-nostr' first." >&2
    exit 1
fi
