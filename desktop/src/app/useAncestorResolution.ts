...
/**
 * Resolves missing ancestor events (thread parents/roots) for the active
 * channel's message timeline and merges them into the query cache.
 */
export function useAncestorResolution(
  ...
  // Reset the dedup set when the active channel changes so ancestor fetches
  // are retried when revisiting a channel (matches pre-extraction behavior).
  ...
  return requestedAncestorIdsRef;
}
