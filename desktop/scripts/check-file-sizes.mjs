...
const overrides = new Map([
  ...
  ["src-tauri/src/managed_agents/persona_card.rs", 978], // PNG/ZIP/MD persona card codec + pack-zip detection + nested root finder + runtime/model/namePool fields + backward-compat provider key read + 28 unit tests
  ...
  ["src-tauri/src/migration.rs", 1120], // worktree shared-agent-data symlink sync (SHARED_AGENT_FILES + SHARED_AGENT_DIRS symlink-to-canonical + sibling pack migration) + mcp_command runtime reconciliation + persona_pack_path reconciliation + provider→runtime persona migration + tests
  ...
  ["src-tauri/src/managed_agents/runtime.rs", 1140], // ... + respond-to gate env (SPROUT_ACP_RESPOND_TO[_ALLOWLIST]) + per-mode env builder + tests + persona/agent env_vars spawn merge (helper + tests now in env_vars.rs) + runtime_metadata_env_vars pure helper + 3 tests
  ...
  ["src-tauri/src/lib.rs", 736], // +4 lines for PairingHandle managed state + 3 pairing command registrations + parse_message_deep_link helper extracted with 6 unit tests covering empty-param filter regression + mod migration + sync_shared_agent_data/reconcile_provider_mcp_commands/reconcile_persona_pack_paths/migrate_persona_provider_to_runtime calls on launch
  ["src/shared/api/tauri.ts", 1212], // pairing command wrappers + applyWorkspace + NIP-44 encrypt/decrypt wrappers + observer_url field + relay member API functions (list/get/add/remove/change-role) + prevent sleep + AcpRuntimeCatalogEntry raw types + fromRawAcpRuntimeCatalogEntry converter + installAcpRuntime
]);
