...
const overrides = new Map([
  ...
  ["src-tauri/src/lib.rs", 860], // +4 lines for PairingHandle managed state + 3 pairing command registrations + parse_message_deep_link helper extracted with 6 unit tests covering empty-param filter regression + mod migration + sync_shared_agent_data/reconcile_provider_mcp_commands/reconcile_persona_pack_paths calls on launch + SIGINT/SIGTERM/SIGHUP signal handlers for agent process cleanup + default first-run window sizing + camera permission commands
  ["src/features/onboarding/ui/AvatarStep.tsx", 1520], // arcade avatar creator with gallery assets, emoji picker, hue/saturation controls, upload fallback, and profile preview
  ["src/features/onboarding/ui/OnboardingFlow.tsx", 870], // multi-step arcade onboarding shell, profile persistence, identity import, welcome return path, and completion routing
  ["src/features/onboarding/ui/TeamTasksStep.tsx", 550], // interactive agent-team onboarding demo with prompt classification and scripted agent responses
  ["src/shared/api/tauri.ts", 1230], // pairing command wrappers + applyWorkspace + NIP-44 encrypt/decrypt wrappers + observer_url field + relay member API functions (list/get/add/remove/change-role) + prevent sleep + AcpProviderCatalogEntry raw types + fromRawAcpProviderCatalogEntry converter + installAcpRuntime + camera permission commands
]);
