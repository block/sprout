...
const overrides = new Map([
  ...
  ["src/shared/api/relayClientSession.ts", 910], // durable websocket session manager with reconnect/replay/recovery state + sendTypingIndicator + fetchChannelHistoryBefore + subscribeToChannelLive (huddle TTS) + subscribeToHuddleEvents (huddle indicator) + disconnect() for workspace switch teardown + NIP-RS read state fetch/subscribe/publish + fetchOwnReadStateBlob
