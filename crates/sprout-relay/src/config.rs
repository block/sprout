...
pub struct Config {
    ...
    ///
    /// On open relays (`require_relay_membership = false`), this flag also
    /// enables owner extraction from NIP-OA auth tags during WebSocket auth,
    /// allowing agent→owner backfill for observer frame authorization.
    ///
