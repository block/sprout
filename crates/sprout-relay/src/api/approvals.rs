...
//!
//! Endpoints:
//!   POST /api/approvals/:token/grant          — grant a pending approval (plaintext token)
//!   POST /api/approvals/:token/deny           — deny a pending approval (plaintext token)
//!   POST /api/approvals/by-hash/:hash/grant   — grant a pending approval (stored hash)
//!   POST /api/approvals/by-hash/:hash/deny    — deny a pending approval (stored hash)
...
pub async fn grant_approval_by_hash(
    ...
    let token_hash = hex::decode(&hash_hex)
        .map_err(|_| ApiError::BadRequest("invalid hex token hash".into()))?;
