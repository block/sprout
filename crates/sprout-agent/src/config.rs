...
/// Text is governed by the much smaller `SPROUT_AGENT_MAX_TOOL_RESULT_TEXT_BYTES`.
...
/// 10 KB. Tunable via `SPROUT_AGENT_MAX_TOOL_RESULT_TEXT_BYTES`.
...
pub struct Config {
    ...
    /// Set via `SPROUT_AGENT_MAX_TOOL_RESULT_TEXT_BYTES`.
...
impl Config {
    pub fn from_env() -> Result<Self, String> {
        ...
        let cfg = Config {
            ...
            max_tool_result_text_bytes: parse_env(
                "SPROUT_AGENT_MAX_TOOL_RESULT_TEXT_BYTES",
    ...
    fn validate(&self) -> Result<(), String> {
        ...
        {
            return Err(format!(
                "config: SPROUT_AGENT_MAX_TOOL_RESULT_TEXT_BYTES must be in {MIN_TOOL_RESULT_TEXT_BYTES}..={MAX_TOOL_RESULT_BYTES}"
            ));
        }
