...
A client sends `session/prompt`. The agent loops: call the LLM → get tool calls → ask for permission → run them via MCP → feed results back → repeat. The loop terminates when the LLM stops asking for tools, the round cap is hit, or the client cancels.
...
// 5. Agent asks for permission. This is a request — it expects a response.
←  {"jsonrpc":"2.0","id":1000000,"method":"session/request_permission","params":{
    "sessionId":"ses_a1b2c3d4e5f6a7b8",
    "toolCall":{"toolCallId":"toolu_01XYZ","title":"echo.say","kind":"other"},
    "options":[
...
// 6. Client approves.
→ {"jsonrpc":"2.0","id":1000000,"result":{"outcome":{"outcome":"selected","optionId":"allow"}}}

// 7. Agent moves the call to in_progress, runs the MCP tool, then completed.
← {"jsonrpc":"2.0","method":"session/update","params":{
    "sessionId":"ses_a1b2c3d4e5f6a7b8",
...
| `SPROUT_AGENT_TOOL_TIMEOUT_SECS` | `120` | |
...
| MCP child lifetime | Process group via `setpgid(0,0)` in `pre_exec`. On timeout, transport break, or shutdown: `killpg(SIGKILL)`. Grandchildren die too. |
| Server poisoning | After a timeout or transport break, the offending server is fenced. Future calls fail fast with `server unavailable after timeout`. Other servers keep working. |
...
| One in flight | One session per process; one prompt per session. Concurrent attempts get `-32602`. |
...
| Tool call timeout | 120 s | `SPROUT_AGENT_TOOL_TIMEOUT_SECS` |
...
One reader, one writer, one prompt task at a time. Permission round-trips ride on a `HashMap<i64, oneshot::Sender>` — the agent loop inserts an entry, the reader's permission-response handler removes and resolves it.
...
- **Regression tests are the changelog.** Each `#[test]` in `regressions.rs` is named for the bug it locks down: `assistant_text_preserved_across_prompts`, `cancel_leaves_history_valid_for_next_prompt`, `grandchild_killed_on_tool_timeout`, `oversize_line_kills_connection`. Read them in order to learn the protocol's failure modes.

The `grandchild_killed_on_tool_timeout` test alone is worth the price of admission — it spawns an MCP server that forks a grandchild that ignores `SIGTERM`, triggers a tool timeout, and verifies the grandchild dies anyway via process-group `SIGKILL`. Most agents leak that grandchild.
