...
async fn main() {
    ...
    drop(shadow_keys); // not threaded through ProxyState yet; constructed to validate salt
    ...
    // upstream_tx / upstream_rx: server → upstream (outbound JSON strings)
    let (upstream_tx, _upstream_rx) = mpsc::channel::<String>(1024);
    ...
    let upstream = UpstreamClient::new(upstream_url.clone(), api_token.clone());
