...
impl Db {
    ...
    pub async fn replace_addressable_event(
        ...
        // Fetch the newest active row for this addressable key.
        // ORDER BY created_at DESC, id DESC: NIP-01 tiebreak (higher ID wins at same timestamp).
        // Handles legacy corruption where multiple active rows exist for the same addressable key.
