...
mod tests {
    ...
    async fn test_send_event_rejects_wrong_pubkey() {
        ...
        let event = EventBuilder::new(Kind::TextNote, "wrong author").tags( [])
    ...
    mod integration {
        ...
        async fn send_event_receives_ok_response() {
            ...
            let event = EventBuilder::new(Kind::Custom(9), "test").tags( [])
                .sign_with_keys(&keys)
                .unwrap();
                ...
                while let Some(Ok(msg)) = stream.next().await {
                    if let Message::Text(text) = msg {
                        ...
                        if arr.first().and_then(|v| v.as_str()) == Some("REQ") {
                            ...
                            for i in 0u8..3 {
                                let ev = EventBuilder::new(Kind::TextNote, format!("msg {i}")).tags( [])
        ...
        async fn send_event_times_out_when_no_ok() {
            ...
            let event = EventBuilder::new(Kind::Custom(9), "timeout-test").tags( [])
                .sign_with_keys(&keys)
                .unwrap();
        ...
        async fn bg_reconnects_on_transport_close() {
            ...
            let event = EventBuilder::new(Kind::Custom(9), "after-reconnect").tags( [])
                .sign_with_keys(&keys)
                .unwrap();
            let ok = client.send_event(event).await.unwrap();
            assert!(ok.accepted);
