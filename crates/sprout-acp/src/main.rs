...
use queue::{parse_thread_tags, EventQueue, QueuedEvent};
...
async fn tokio_main() -> Result<()> {
    ...
    // Channels where an agent turn is in flight, mapped to the NIP-10 parent of
    // the prompt's first event (for scoped kind:20002), or `None` if unknown.
    let mut typing_channels: HashMap<Uuid, Option<String>> = HashMap::new();
    ...
    loop {
        ...
        if last_maintenance.elapsed() >= maintenance_interval {
            ...
            if queue.has_flushable_work() {
                for (ch, parent) in dispatch_pending(&mut pool, &mut queue, &ctx) {
                    typing_channels.insert(ch, parent);
                }
            }
        }
        ...
        let pool_event: Option<PoolEvent> = {
            ...
            tokio::select! {
                ...
                _ = async {
                    ...
                    for (ch, reply_parent) in &typing_channels {
                        if let Ok(event) = relay.build_typing_event(*ch, reply_parent.as_deref()) {
...
fn dispatch_pending(
    ...
    loop {
        ...
        let reply_parent = batch
            .events
            .first()
            .and_then(|be| parse_thread_tags(&be.event).parent_event_id);
        ...
        dispatched_channels.push((channel_id, reply_parent));
    }
    tracing::debug!(
...
fn recover_panicked_agent(
    ...
    heartbeat_in_flight: &mut bool,
    removed_channels: &HashSet<Uuid>,
    typing_channels: &mut HashMap<Uuid, Option<String>>,
    crash_history: &mut [SlotCircuit],
    respawn_tx: &mpsc::Sender<RespawnResult>,
    respawn_tasks: &mut tokio::task::JoinSet<()>,
) {
