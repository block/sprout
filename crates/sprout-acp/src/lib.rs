                        ...
                        Some(sprout_event) => {
                            ...
                            if is_cancel {
                                if let Some(owner) = owner_cache.get() {
                                    if sprout_event.event.pubkey.to_hex() == *owner {
                                        let fired = signal_in_flight_task(&mut pool, sprout_event.channel_id, ControlSignal::Cancel);
                                        if !fired {
                                            tracing::warn!(
                                                channel_id = %sprout_event.channel_id,
                                ...
                                if let Some(owner) = owner_cache.get() {
                                    if sprout_event.event.pubkey.to_hex() == *owner {
                                        ...
                                        if fired {
                                            ...
                                            let invalidated = pool.invalidate_channel_sessions(sprout_event.channel_id);
                                            tracing::info!(
                                                channel_id = %sprout_event.channel_id,
                                                invalidated,
                            ...
                            if accepted && queue.is_channel_in_flight(sprout_event.channel_id) {
                                ...
                                if should_cancel {
                                    signal_in_flight_task(&mut pool, sprout_event.channel_id, ControlSignal::Interrupt);
                                }
                            }
                            // ── End mode gate ────────────────────────────────
