...
async fn test_upload_idempotent() {
    ...
    assert_eq!(
        r1["url"], r2["url"],
        "url must be identical on re-upload"
    );
}
...
async fn test_upload_real_image() {
    ...
    let descriptor: serde_json::Value = serde_json::from_str(&body_text).expect("BlobDescriptor JSON");
    println!("BlobDescriptor: {descriptor:#}");
    ...
    assert!(descriptor["dim"].as_str().is_some(), "real image should have dim");
    assert!(descriptor["blurhash"].as_str().is_some(), "real image should have blurhash");
    ...
    assert_eq!(returned.as_ref(), bytes.as_slice(), "GET must return original bytes");
